...
const readCzConfig = () => {
  ...
  const czConfig = findConfig.require(".cz-config.js", { home: false });
  ...
  if (
    ...
    console.error(
      chalk.bold.white.bgRed(
        "cz-customisable config not specified in package.json!"
      )
    );
    return false;
  }

  czConfig = readCzConfig();
  return true;
}
...
function commitError() {
  ...
  console.error(
    chalk.bold.white.bgRed("INVALID COMMIT MSG") +
      ": " +
...
function validateMessage(firstLine, fullMsg) {
  ...
  allowCustomScopes =
    czConfig.allowCustomScopes !== undefined
      ? !!czConfig.allowCustomScopes
      : false; // Convert to boolean. If undefined, do not allow custom scopes.
  ...
  if (!match) {
    commitError(
      `Does not match "<type>(<scope>): <subject-starting-with-lowercase-letter>"! Was: ${firstLine}`
    );
    return false;
  }
  ...
  allowedScopesForType = allowedScopesForType
    .map((item) => item.name)
    .concat(SCOPES);
  ...
  if (
    ...
    !SCOPES.length &&
    !allowCustomScopes &&
    allowedScopesForType.indexOf(scope) === -1
  ) {
    commitError(
  ...
  if (
    ...
    commitError(
      `"${scope}" is not allowed scope for a "${type}" commit!\nValid "${type}" scopes: ${allowedScopesForType
        .sort()
        .join(", ")}`
    );
    return false;
  ...
  if (!subject.trim().length) {
    commitError(
      `Subject must be a word that is at least ${MIN_SUBJECT_LENGTH} characters long`
    );
    return false;
  }
  ...
  if (!subject.length || subject[0] === subject[0].toUpperCase()) {
    commitError(
      `"${subject.trim()}" must start with a lower-case character (and use imperative language)`
    );
    return false;
  }
  ...
  if (subject.split(" ")[0].length < MIN_SUBJECT_LENGTH) {
    commitError(
      `"${subject.trim()}" must start with a word that is at-least ${MIN_SUBJECT_LENGTH} characters long (and use imperative language)`
    );
    return false;
  }
  ...
  if (
    BREAKING_CHANGE_PATTERN.test(fullMsg) &&
    allowBreakingChangesForType.indexOf(type) === -1
  ) {
    commitError(
      `Breaking changes are not permitted for this type. Valid types: ${allowBreakingChangesForType.join(
        ", "
      )}`
    );
    return false;
...
function processCLI(commitMsgFileName, cb) {
  ...
  let incorrectLogFileName = commitMsgFileName.replace(
    "COMMIT_EDITMSG",
  ...
  appendBranchNameToCommitMessage =
    czConfig && czConfig.appendBranchNameToCommitMessage != undefined
      ? czConfig.appendBranchNameToCommitMessage
      : appendBranchNameToCommitMessage;
...
if (process.argv.join("").indexOf("mocha") === -1) {
  ...
  processCLI(
    process.argv[2] || process.env.GIT_PARAMS || process.env.HUSKY_GIT_PARAMS
  ); // GIT_PARAMS are made available when using husky
} else {
